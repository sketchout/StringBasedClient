...
public class StringBasedClient
{
	...
	public void sendMessage( String message ) throws IOException
	{
		...
		int iRemaining = cb.remaining();
		bb.putInt( iRemaining ); // Writes four bytes containing the given int value
		...
		//String s = new String(bb.array(), "ISO-8859-1");
		logger.error( "[bb.putInt : (" + iRemaining +") => bb["+ Arrays.toString(bytes) +"]"  );

		// byte[] b = new byte[ bb.remaining() ];
		// bb.get( b, 0, b.length);
		// String v = new String( b, Charset.forName("ISO-8859-1") );
		// logger.error( "[bb.putInt : cb.remaining(" + iRemaining +") => bb["+ v +"]"  );

		
		int numWritten = sc.write( bb );

		
		// maximumMessagePrefixToLog = 60;
		...
		logger.error( "[Connector:" + title + "]    wrote " + numWritten + " bytes (" + messageWritten + ")" );
	}
